...
@Repository
public interface OrdinateurRespoRepository extends JpaRepository<Ordinateur,Integer> {
...
//List<Ordinateur> findOrdinateurByDepartement(String departement);
 ...
 /*******************************************************************/
 @Query(nativeQuery = true,value = "" +
         "select im.*,m.code_barre,verifie,m.date_livraison,duree_garentie,marque,prix,appel_offre_id,ensiegnant_id from materiel m,ordinateur im where im.id=m.id and m.verifie=1 and m.appel_offre_id IS null GROUP BY im.id")
 List<Ordinateur> getBesoinOrd();
 @Query(nativeQuery = true,value = "" +
         "select im.*,m.code_barre,verifie,m.date_livraison,duree_garentie,matp.marque," +
 ...
 @Query(nativeQuery = true,value = "" +
         ...
         "              appel_offre_id,ensiegnant_id,m.prix,m.marque" +
